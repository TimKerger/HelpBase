...
# Description: Install and manage a HelpBase installation.
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/helpbase-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for HelpBase (e.g., helpbase.domain.com): ' domain_name
...
serve HelpBase at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "helpbase"; then
    adduser --disabled-password --gecos "" helpbase
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/helpbase/config
  file="/opt/helpbase/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/helpbase/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create helpbase db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER helpbase CREATEDB;
    ALTER USER helpbase PASSWORD :'pass';
    ALTER ROLE helpbase SUPERUSER;
...
#   None
##############################################################################
function setup_helpbase() {
  local secret=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 63 ; echo '')
  local RAILS_ENV=production
  get_pgpass

  sudo -i -u helpbase << EOF
  rvm --version
  rvm autolibs disable
  rvm install "ruby-3.4.4"
  rvm use 3.4.4 --default

  git clone https://github.com/TimKerger/HelpBase.git helpbase
  cd helpbase
  git checkout "$BRANCH"
  bundle
  pnpm i
...
##############################################################################
# Setup HelpBase systemd services and cwctl CLI
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function configure_systemd_services() {
  ...
  if [ -f "/etc/systemd/system/helpbase.target" ]; then
  ...
  if [ "$DEPLOYMENT_TYPE" == "web" ]; then
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop helpbase.target || true
      systemctl disable helpbase.target || true
      systemctl stop helpbase-worker.1.service || true
      systemctl disable helpbase-worker.1.service || true
    fi

    # Stop and disable worker target if converting from worker-only
    if [ -f "/etc/systemd/system/helpbase-worker.target" ]; then
      echo "Converting from worker-only to web-only"
      systemctl stop helpbase-worker.target || true
      systemctl disable helpbase-worker.target || true
    fi

    cp /home/helpbase/helpbase/deployment/helpbase-web.1.service /etc/systemd/system/helpbase-web.1.service
    cp /home/helpbase/helpbase/deployment/helpbase-web.target /etc/systemd/system/helpbase-web.target

    systemctl daemon-reload
    systemctl enable helpbase-web.target
    systemctl start helpbase-web.target
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop helpbase.target || true
      systemctl disable helpbase.target || true
      systemctl stop helpbase-web.1.service || true
      systemctl disable helpbase-web.1.service || true
    fi

    # Stop and disable web target if converting from web-only
    if [ -f "/etc/systemd/system/helpbase-web.target" ]; then
      echo "Converting from web-only to worker-only"
      systemctl stop helpbase-web.target || true
      systemctl disable helpbase-web.target || true
    fi

    cp /home/helpbase/helpbase/deployment/helpbase-worker.1.service /etc/systemd/system/helpbase-worker.1.service
    cp /home/helpbase/helpbase/deployment/helpbase-worker.target /etc/systemd/system/helpbase-worker.target

    systemctl daemon-reload
    systemctl enable helpbase-worker.target
    systemctl start helpbase-worker.target
    ...
    cp /home/helpbase/helpbase/deployment/helpbase-web.1.service /etc/systemd/system/helpbase-web.1.service
    cp /home/helpbase/helpbase/deployment/helpbase-worker.1.service /etc/systemd/system/helpbase-worker.1.service
    cp /home/helpbase/helpbase/deployment/helpbase.target /etc/systemd/system/helpbase.target

    systemctl daemon-reload
    systemctl enable helpbase.target
    systemctl start helpbase.target
  fi

  cp /home/helpbase/helpbase/deployment/helpbase /etc/sudoers.d/helpbase
  cp /home/helpbase/helpbase/deployment/setup_20.04.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/TimKerger/HelpBase/master/deployment/nginx_helpbase.conf
  cp nginx_helpbase.conf /etc/nginx/sites-available/nginx_helpbase.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/helpbase.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_helpbase.conf
  ln -s /etc/nginx/sites-available/nginx_helpbase.conf /etc/nginx/sites-enabled/nginx_helpbase.conf
...
EOF
  ...
  # Restart the appropriate helpbase target
  if [ -f "/etc/systemd/system/helpbase-web.target" ]; then
    systemctl restart helpbase-web.target
  elif [ -f "/etc/systemd/system/helpbase-worker.target" ]; then
    systemctl restart helpbase-worker.target
  else
    systemctl restart helpbase.target
  fi
}
...
function setup_logging() {
  touch /var/log/helpbase-setup.log
  LOG_FILE="/var/log/helpbase-setup.log"
}
...
***************************************************************************
Woot! Woot!! HelpBase server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://helpbase.app/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new HelpBase CLI tool to manage your installation.'
...
***************************************************************************
              HelpBase Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/helpbase-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for HelpBase?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing HelpBase. This takes a long while."
  setup_helpbase &>> "${LOG_FILE}"
...
https://www.helpbase.app/docs/deployment/deploy-helpbase-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u helpbase bash -c " cd helpbase && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your HelpBase installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of HelpBase
  -I BRANCH                 Install HelpBase from a git branch
  -u, --upgrade             Upgrade HelpBase to the latest stable version
  -U BRANCH                 Upgrade HelpBase from a git branch (EXPERIMENTAL)
...
Management:
  ...
  -l, --logs                View logs from HelpBase. Supported values include web/worker.
  -r, --restart             Restart HelpBase server
...
Report bugs at https://github.com/TimKerger/HelpBase/issues
Get help, https://helpbase.app/community?utm_source=cwctl

EOF
}

##############################################################################
# Get HelpBase web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u helpbase-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u helpbase-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(HelpBase v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from HelpBase 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading HelpBase to v$CW_VERSION (branch: $BRANCH)"
...
EOF
  ...
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "HelpBase v4.0 and above requires pgvector support in PostgreSQL."
  ...
  # Navigate to the HelpBase directory
  cd helpbase
...
##############################################################################
# Restart HelpBase server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  if [ -f "/etc/systemd/system/helpbase-web.target" ]; then
    systemctl restart helpbase-web.target
    systemctl status helpbase-web.target
  elif [ -f "/etc/systemd/system/helpbase-worker.target" ]; then
    systemctl restart helpbase-worker.target
    systemctl status helpbase-worker.target
  else
    systemctl restart helpbase.target
    systemctl status helpbase.target
  fi
}

##############################################################################
# Convert existing HelpBase deployment to different type (--convert)
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function convert_deployment() {
  echo "Converting HelpBase deployment to: $DEPLOYMENT_TYPE"

  # Check if HelpBase is installed
  if [ ! -d "/home/helpbase/helpbase" ]; then
    echo "HelpBase installation not found. Use --install first."
    exit 1
  fi
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u helpbase << "EOF"
  cd helpbase
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/TimKerger/HelpBase/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://get.helpbase.app/linux/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
